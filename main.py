...
if __name__ == "__main__":
    ...
    print(f"\nCombattimento tra {p1.get_nome()} e {p2.get_nome()}!")
    print(f"{p1.get_nome()} usa {p1.get_arma()}")
    print(f"{p2.get_nome()} usa {p2.get_arma()}\n")
    ...
    while p1.e_vivo() and p2.e_vivo():
        ...
        print(f"{p1.get_nome()} infligge {danno1} danni a {p2.get_nome()}. ({p2})")

        if not p2.e_vivo():
            break  

        danno2 = p2.attacca(p1)
        print(f"{p2.get_nome()} infligge {danno2} danni a {p1.get_nome()}. ({p1})\n")
